...
public class AlamatController {
    ...
    @GetMapping("/alamat/form")
    public ModelMap tampilkanForms(@RequestParam(value = "id", required = false) Alamat alamat, Model karyawan) {
        if (alamat == null) {
            alamat = new Alamat();
        }
        karyawan.addAttribute("alamat", alamat);
        karyawan.addAttribute("karyawan", karyawanDao.findAll());
        return new ModelMap("alamat", alamat);
        // return "alamat/form";
    }
    // @GetMapping("/alamat/form")
    // public String tampilkanForms(@RequestParam(value = "id", required = false)
    // Alamat alamat, Model m) {
    // if (alamat == null) {
    // alamat = new Alamat();
    // }
    ...
    @PostMapping("/alamat/form")
    public String simpan(@Valid @ModelAttribute("alamat") Alamat alamat, @ModelAttribute("karyawan") Karyawan karyawan,
            BindingResult errors,
            SessionStatus status) {
        if (errors.hasErrors()) {
            return "alamat/form";
        ...
        // Query q = session.createQuery("update alamat set
        // nama=:n,alamat=:o,id_karyawan=:p where id=:i");
